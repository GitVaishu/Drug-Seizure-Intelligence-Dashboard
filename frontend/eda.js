...
function initEDA() {
  ...
  document.querySelectorAll('.eda-tab').forEach(tab => {
    tab.addEventListener('click', () => {
      ...
      document.getElementById('eda-panel-' + tab.dataset.eda).classList.add('active');
      const key = '_eda_'+tab.dataset.eda;
      if (!window[key]) { renderSection(tab.dataset.eda); window[key]=true; }
    });
  });
  ...
  document.querySelectorAll('.eda-chart-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      panel.querySelectorAll('.eda-chart-btn').forEach(b=>b.classList.remove('active'));
      panel.querySelectorAll('.eda-chart-view').forEach(v=>v.classList.remove('active'));
...
function renderSection(s) {
  const map = {
    time:'renderTime', geo:'renderGeo', drug:'renderDrug',
    enforcement:'renderEnforcement', border:'renderBorder', advanced:'renderAdvanced'
  };
  if(map[s]) { window[map[s]](); buildInferences(s); }
}
...
function renderTime() {
  const years  = YEARS;
  const kgYr   = years.map(y => sumBy(RAW_DATA.filter(d=>d.Year===y),'Total_Kg'));
  const casesYr= years.map(y => sumBy(RAW_DATA.filter(d=>d.Year===y),'Total_Cases'));

  /* Chart 1 — Total Seizures */
  Plotly.newPlot('chart-time-total',[
    { x:years, y:kgYr, type:'scatter', mode:'lines+markers', name:'Kg Seized',
      line:{color:'#f97316',width:3},
      marker:{size:10,color:'#f97316',line:{color:'#fff',width:2}},
      fill:'tozeroy', fillcolor:'rgba(249,115,22,0.09)',
      hovertemplate:'Year: <b>%{x}</b><br>Seized: <b>%{y:,.0f} Kg</b><extra></extra>' },
    { x:years, y:casesYr, type:'scatter', mode:'lines+markers', name:'Cases Filed',
      line:{color:'#38bdf8',width:2,dash:'dot'}, marker:{size:7}, yaxis:'y2',
      hovertemplate:'Year: <b>%{x}</b><br>Cases: <b>%{y:,}</b><extra></extra>' }
  ],{
    ...L,
    title:{text:'Total Drug Seizures & Cases Filed by Year'},
    xaxis:{...L.xaxis, title:'Year (2018–2024)', dtick:1, tickformat:'d'},
    yaxis:{...L.yaxis, title:'Total Seizure Weight (Kg)'},
    yaxis2:{...L.yaxis, title:'Number of Cases Filed', overlaying:'y', side:'right'},
    legend:{...L.legend, x:0.01,y:0.99}
  },C);

  /* Chart 2 — Drug-wise trends: top 6 by total Kg */
  const top6 = [...DKG].sort((a,b)=>sumBy(RAW_DATA,b.col)-sumBy(RAW_DATA,a.col)).slice(0,6);
  const traces2 = top6.map(d=>({
    x:years,
    y:years.map(y=>sumBy(RAW_DATA.filter(r=>r.Year===y),d.col)),
    name:d.label, type:'scatter', mode:'lines+markers',
  ...
  const grYr = years.slice(1).map(y => {
    const rows = RAW_DATA.filter(d=>d.Year===y && d.Growth_Rate_Pct !== undefined);
  ...
  Plotly.newPlot('chart-time-growth',[{
    x:years.slice(1), y:grYr, type:'bar',
    marker:{color:grYr.map(v=>v>=0?'#a3e635':'#ef4444'), opacity:0.85},
    hovertemplate:'Year: <b>%{x}</b><br>Median Growth: <b>%{y:.1f}%</b><extra></extra>'
  }],{
    ...L,
    title:{text:'Year-on-Year Median Growth Rate (%) in Seizures'},
    xaxis:{...L.xaxis, title:'Year', dtick:1, tickformat:'d'},
    yaxis:{...L.yaxis, title:'Median Growth Rate (%) across states',
           zeroline:true, zerolinecolor:'#5a7291', zerolinewidth:1}
  },C);
  ...
  Plotly.newPlot('chart-time-ma',[
    { x:years, y:kgYr, name:'Actual Seizures', type:'scatter', mode:'lines+markers',
      line:{color:'#38bdf8',width:2,dash:'dot'}, marker:{size:7, color:'#38bdf8'},
      hovertemplate:'Year %{x}: <b>%{y:,.0f} Kg</b><extra></extra>' },
    { x:years, y:ma3, name:'3-Year Moving Average', type:'scatter', mode:'lines',
      line:{color:'#f97316',width:3},
...
function renderGeo() {
  ...
  const stData = STATES.map(s=>{
    ...
    return { s, risk:rows.reduce((sum,d)=>sum+d.Risk_Index,0), kg:sumBy(rows,'Total_Kg') };
  }).sort((a,b)=>b.risk-a.risk);

  Plotly.newPlot('chart-geo-heat',[{
    y:stData.map(d=>d.s), x:stData.map(d=>d.risk),
    type:'bar', orientation:'h',
    customdata:stData.map(d=>d.kg),
    marker:{
      color:stData.map(d=>d.risk), colorscale:'YlOrRd', showscale:true,
      colorbar:{title:{text:'Risk',font:{color:'#5a7291'}}, tickfont:{color:'#5a7291'}}
    },
    hovertemplate:'<b>%{y}</b><br>Risk Index: %{x:.3f}<br>Total Kg: %{customdata:,.0f}<extra></extra>'
  }],{
    ...L,
    title:{text:'Cumulative State Risk Index (Weighted: Kg 40% + Cases 30% + Diversity 30%)'},
    xaxis:{...L.xaxis, title:'Cumulative Risk Index (sum across 2018–2024)'},
    yaxis:{...L.yaxis, title:'State / UT'},
    margin:{...L.margin, l:200}, height:620
  },C);

  /* Chart 2 — Top 10 States bar */
  const top10 = stData.slice(0,10);
  Plotly.newPlot('chart-geo-top10',[{
    x:top10.map(d=>d.s), y:top10.map(d=>d.kg), type:'bar',
    marker:{color:top10.map((_,i)=>`hsl(${15+i*10},80%,52%)`), opacity:0.88},
    hovertemplate:'<b>%{x}</b><br>Total Seized: %{y:,.0f} Kg<extra></extra>'
  }],{
    ...L,
  ...
  Plotly.newPlot('chart-geo-yearstate',[{
    ...
    hovertemplate:'State: <b>%{y}</b><br>Year: <b>%{x}</b><br>Seized: <b>%{z:,.0f} Kg</b><extra></extra>',
    colorbar:{title:{text:'Kg',font:{color:'#5a7291'}}, tickfont:{color:'#5a7291'}}
  }],{
    ...L,
    title:{text:'State × Year Seizure Heatmap (Kg) — Colour = Volume Intensity'},
    xaxis:{...L.xaxis, title:'Year', dtick:1, tickformat:'d'},
    yaxis:{...L.yaxis, title:'State / UT', tickfont:{size:10}},
    height:680, margin:{...L.margin, l:200}
  },C);
  ...
  Plotly.newPlot('chart-geo-rank',[{
    ...
    hovertemplate:'Rank %{x} — <b>%{text}</b><br>%{y:,.0f} Kg seized<extra></extra>'
  }],{
    ...L,
...
function renderDrug() {
  ...
  const totals = DKG.map(d=>({ label:d.label, val:sumBy(RAW_DATA,d.col) }))
    .filter(d=>d.val>0).sort((a,b)=>b.val-a.val);

  Plotly.newPlot('chart-drug-dist',[{
    x:totals.map(d=>d.label), y:totals.map(d=>d.val), type:'bar',
    marker:{color:totals.map((_,i)=>`hsl(${30+i*15},75%,52%)`), opacity:0.88},
    hovertemplate:'<b>%{x}</b><br>Total Seized: %{y:,.0f} Kg<extra></extra>'
  }],{
    ...L,
    title:{text:'Total Kg Seized by Drug Type (2018–2024, All States)'},
    xaxis:{...L.xaxis, title:'Drug / Substance', tickangle:-30},
    yaxis:{...L.yaxis, title:'Total Kg Seized (log scale)',
           type:'log', tickformat:'.2s'}
  },C);

  /* Chart 2 — Stacked bar year-wise */
  const top5 = [...DKG].sort((a,b)=>sumBy(RAW_DATA,b.col)-sumBy(RAW_DATA,a.col)).slice(0,5);
  const traces2 = top5.map(d=>({
    name:d.label, x:YEARS,
    y:YEARS.map(y=>sumBy(RAW_DATA.filter(r=>r.Year===y),d.col)),
    type:'bar',
  ...
  Plotly.newPlot('chart-drug-stacked', traces2,{
    ...L,
    title:{text:'Year-wise Drug Seizure Composition (Top 5 Substances, Stacked)'},
    barmode:'stack',
    xaxis:{...L.xaxis, title:'Year', dtick:1, tickformat:'d'},
  ...
  const activeDrugs = DKG.filter(d=>sumBy(RAW_DATA,d.col)>100).slice(0,9);
  const pearson = (a,b) => {
  ...
  Plotly.newPlot('chart-drug-corr',[{
    z:corr, x:labs, y:labs, type:'heatmap',
    colorscale:'RdBu', zmid:0, zmin:-1, zmax:1,
    text:corr.map(row=>row.map(v=>v.toFixed(2))),
    texttemplate:'%{text}', textfont:{size:10},
    hovertemplate:'%{y} × %{x}<br>Pearson r = %{z:.3f}<extra></extra>',
    colorbar:{title:{text:'r',font:{color:'#5a7291'}}, tickfont:{color:'#5a7291'}}
  }],{
    ...L,
    title:{text:'Drug Seizure Correlation Matrix (Pearson r) — 1 = perfectly correlated'},
  ...
  Plotly.newPlot('chart-drug-diversity',[{
    x:RAW_DATA.map(d=>d.Drug_Diversity), type:'histogram', nbinsx:14,
    marker:{color:'#a3e635', opacity:0.82, line:{color:'#0f1623',width:1}},
    hovertemplate:'Diversity Score %{x}<br>Count: %{y} state-year records<extra></extra>'
  }],{
    ...L,
    title:{text:'Drug Diversity Score Distribution — How Many Drug Types per State-Year?'},
    xaxis:{...L.xaxis, title:'Drug Diversity Score (# of drug types with seizures > 0)', dtick:1},
...
function renderEnforcement() {
  ...
  Plotly.newPlot('chart-enf-scatter',[{
    ...
    mode:'markers',
    marker:{
      color:RAW_DATA.map(d=>d.Risk_Index), colorscale:'Plasma', opacity:0.72, size:9,
      colorbar:{title:{text:'Risk',font:{color:'#5a7291'}}, tickfont:{color:'#5a7291'}}
    },
    hovertemplate:'<b>%{text}</b><br>Cases: %{x:,}<br>Arrested: %{y:,}<extra></extra>'
  }],{
    ...L,
    title:{text:'Cases Filed vs Persons Arrested — Each Point = One State-Year'},
  ...
  const rates = RAW_DATA.map(d=>d.Arrest_Rate).filter(r=>r>0 && r<5);
  Plotly.newPlot('chart-enf-dist',[{
    x:rates, type:'histogram', nbinsx:25,
    marker:{color:'#38bdf8', opacity:0.82, line:{color:'#0f1623',width:1}},
    hovertemplate:'Arrest Rate: %{x:.2f}x<br>Count: %{y} records<extra></extra>'
  }],{
    ...L,
    title:{text:'Distribution of Arrest Rate Across All State-Year Records'},
    xaxis:{...L.xaxis, title:'Arrest Rate (Persons Arrested ÷ Cases Filed)'},
    yaxis:{...L.yaxis, title:'Number of State-Year Records'}
  },C);

  /* Chart 3 — State-wise avg arrest rate */
  const stRates = STATES.map(s=>{
    const rows=RAW_DATA.filter(d=>d.State_UT===s && d.Arrest_Rate>0 && d.Arrest_Rate<5);
    return { s, r: rows.length ? rows.reduce((sum,d)=>sum+d.Arrest_Rate,0)/rows.length : 0 };
  }).sort((a,b)=>b.r-a.r);

  Plotly.newPlot('chart-enf-state',[{
    y:stRates.map(d=>d.s), x:stRates.map(d=>d.r),
    type:'bar', orientation:'h',
    marker:{color:stRates.map(d=>d.r>=2?'#ef4444':d.r>=1.5?'#f97316':'#38bdf8'), opacity:0.88},
    hovertemplate:'<b>%{y}</b><br>Avg Arrest Rate: %{x:.2f}x<extra></extra>'
  }],{
    ...L,
    title:{text:'Average Arrest Rate by State (2018–2024) — Red > 2x, Orange 1.5–2x, Blue < 1.5x'},
  ...
  const natCases  = YEARS.map(y=>sumBy(RAW_DATA.filter(d=>d.Year===y),'Total_Cases'));
  const natArr    = YEARS.map(y=>sumBy(RAW_DATA.filter(d=>d.Year===y),'Total_Arrested'));
  Plotly.newPlot('chart-enf-trend',[
    { x:YEARS, y:natCases, name:'Cases Filed', type:'scatter', mode:'lines+markers',
      line:{color:'#38bdf8',width:3}, marker:{size:9},
      hovertemplate:'Year %{x}<br>Cases: %{y:,}<extra></extra>' },
    { x:YEARS, y:natArr,   name:'Persons Arrested', type:'scatter', mode:'lines+markers',
      line:{color:'#f97316',width:3}, marker:{size:9},
      hovertemplate:'Year %{x}<br>Arrested: %{y:,}<extra></extra>' }
  ],{
    ...L,
    title:{text:'National Enforcement Trend — Cases Filed vs Persons Arrested by Year'},
    xaxis:{...L.xaxis, title:'Year', dtick:1, tickformat:'d'},
...
function renderBorder() {
  ...
  const bKg=sumBy(RAW_DATA.filter(d=>d.Border_State),'Total_Kg');
  const nKg=sumBy(RAW_DATA.filter(d=>!d.Border_State),'Total_Kg');
  const bC =sumBy(RAW_DATA.filter(d=>d.Border_State),'Total_Cases');
  const nC =sumBy(RAW_DATA.filter(d=>!d.Border_State),'Total_Cases');
  Plotly.newPlot('chart-bord-pie',[
    { type:'pie', values:[bKg,nKg], labels:['Border','Non-Border'], hole:0.45,
      marker:{colors:['#f97316','#38bdf8']}, domain:{x:[0,0.45]},
      textinfo:'label+percent',
      hovertemplate:'<b>%{label}</b><br>%{value:,.0f} Kg (%{percent})<extra></extra>' },
    { type:'pie', values:[bC,nC],  labels:['Border','Non-Border'], hole:0.45,
      marker:{colors:['#f97316','#38bdf8']}, domain:{x:[0.55,1]}, showlegend:false,
      textinfo:'label+percent',
      hovertemplate:'<b>%{label}</b><br>%{value:,} cases (%{percent})<extra></extra>' }
  ],{
    ...L,
    title:{text:'Border vs Non-Border States — Left: Seizure Weight (Kg) | Right: Cases Filed'},
    annotations:[
      {x:0.22,y:0.5,text:'Kg Seized',showarrow:false,font:{color:'#5a7291',size:12}},
      {x:0.78,y:0.5,text:'Cases Filed',showarrow:false,font:{color:'#5a7291',size:12}}
    ],
  ...
  const coastalYr = YEARS.map(y=>({
    c: sumBy(RAW_DATA.filter(d=>d.Year===y && d.Coastal_State), 'Total_Kg'),
    n: sumBy(RAW_DATA.filter(d=>d.Year===y && !d.Coastal_State),'Total_Kg')
  }));
  Plotly.newPlot('chart-bord-coastal',[
    { x:YEARS, y:coastalYr.map(d=>d.c), name:'Coastal States',     type:'bar',
      marker:{color:'#38bdf8',opacity:0.85},
      hovertemplate:'Year %{x}<br>Coastal: %{y:,.0f} Kg<extra></extra>' },
    { x:YEARS, y:coastalYr.map(d=>d.n), name:'Non-Coastal States', type:'bar',
      marker:{color:'#a3e635',opacity:0.85},
      hovertemplate:'Year %{x}<br>Non-Coastal: %{y:,.0f} Kg<extra></extra>' }
  ],{
    ...L,
    title:{text:'Coastal vs Non-Coastal States — Annual Seizure Comparison (Kg)'},
    barmode:'group',
    xaxis:{...L.xaxis, title:'Year', dtick:1, tickformat:'d'},
  ...
  const catKg=[0,0,0,0], catCases=[0,0,0,0];
  RAW_DATA.forEach(d=>{
    const i = d.Border_State && d.Coastal_State ? 0
            : d.Border_State                    ? 1
            : d.Coastal_State                   ? 2 : 3;
    catKg[i]    += d.Total_Kg;
  ...
  Plotly.newPlot('chart-bord-combo',[
    { x:cats, y:catKg,    name:'Kg Seized', type:'bar',
      marker:{color:['#ef4444','#f97316','#38bdf8','#5a7291'],opacity:0.88},
      hovertemplate:'<b>%{x}</b><br>%{y:,.0f} Kg seized<extra></extra>' },
    { x:cats, y:catCases, name:'Cases Filed', type:'bar',
      marker:{color:['#ef4444','#f97316','#38bdf8','#5a7291'],opacity:0.5},
      yaxis:'y2',
      hovertemplate:'<b>%{x}</b><br>%{y:,} cases<extra></extra>' }
  ],{
    ...L,
    title:{text:'Combined Geographic Risk Zone Analysis — Seizures (Kg) and Cases by Zone Type'},
    barmode:'group',
    xaxis:{...L.xaxis, title:'Geographic Zone Classification'},
    yaxis: {...L.yaxis, title:'Total Kg Seized'},
    yaxis2:{...L.yaxis, title:'Cases Filed', overlaying:'y', side:'right'},
    legend:{...L.legend, x:0.01, y:0.99}
  },C);
}
...
function renderAdvanced() {
  ...
  const stByKg = STATES
    .map(s=>({ s, vals:RAW_DATA.filter(d=>d.State_UT===s).map(d=>d.Total_Kg) }))
    .sort((a,b)=>b.vals.reduce((s,v)=>s+v,0)-a.vals.reduce((s,v)=>s+v,0))
    .slice(0,8);

  Plotly.newPlot('chart-adv-box', stByKg.map(d=>({
    y:d.vals, name:d.s.length>14?d.s.slice(0,14)+'…':d.s,
    type:'box', boxpoints:'all', jitter:0.35, pointpos:0,
    marker:{size:5, opacity:0.6}
  })),{
    ...L,
    title:{text:'Seizure Weight Distribution — Top 8 States (Each dot = 1 year)'},
  ...
  const hotspots = STATES.map(s=>{
    const rows = RAW_DATA.filter(d=>d.State_UT===s && d.Growth_Rate_Pct!==undefined
      && Math.abs(d.Growth_Rate_Pct)<500);
    if (!rows.length) return {s, gr:0, yr:'-'};
    const peak = rows.reduce((best,d)=>d.Growth_Rate_Pct>best.Growth_Rate_Pct?d:best, rows[0]);
    return {s, gr:peak.Growth_Rate_Pct, yr:peak.Year};
  }).sort((a,b)=>b.gr-a.gr).slice(0,15);

  Plotly.newPlot('chart-adv-hotspot',[{
    x:hotspots.map(d=>d.s), y:hotspots.map(d=>d.gr),
    text:hotspots.map(d=>`Peak: ${d.yr}`),
    type:'bar',
    marker:{
      color:hotspots.map(d=>d.gr), colorscale:'YlOrRd', showscale:true,
      colorbar:{title:{text:'Growth%',font:{color:'#5a7291'}},tickfont:{color:'#5a7291'}}
    },
    hovertemplate:'<b>%{x}</b><br>Peak Growth: %{y:.1f}%<br>%{text}<extra></extra>'
  }],{
    ...L,
    title:{text:'Growth Hotspots — Top 15 States by Peak Year-on-Year Growth Rate (%)'},
  ...
  const std   = Math.sqrt(kgAll.reduce((s,v)=>s+(v-mean)**2,0)/kgAll.length);
  ...
  Plotly.newPlot('chart-adv-anomaly',[
    { x:normal.map(d=>d.Year), y:normal.map(d=>d.Total_Kg),
      text:normal.map(d=>d.State_UT),
      mode:'markers', name:'Normal (|z| ≤ 2)',
      marker:{color:'#38bdf8', size:7, opacity:0.5},
      hovertemplate:'<b>%{text}</b><br>Year %{x}: %{y:,.0f} Kg<extra></extra>' },
    { x:anomalies.map(d=>d.Year), y:anomalies.map(d=>d.Total_Kg),
    ...
    ...L,
    title:{text:'Anomaly Detection — Statistical Outliers in Seizure Volume (Z-Score Method)'},
    xaxis:{...L.xaxis, title:'Year', dtick:1, tickformat:'d'},
    yaxis:{...L.yaxis, title:'Total Kg Seized (anomalous records marked in red ★)'},
    legend:{...L.legend, x:0.01, y:0.99}
  },C);
}
...
const INFERENCES = {
  time: [
    { tag:'obs',    title:'Rising National Trend Post-2019',
    ...
    { tag:'reason', title:'COVID-19 Disruption Then Rebound',
    ...
    { tag:'impact', title:'Ganja Dominates All Drug Trends',
    ...
    { tag:'obs',    title:'Median Growth Rate Shows Real Volatility',
    ...
    { tag:'reason', title:'Moving Average Confirms Upward Baseline',
  ...
  geo: [
    { tag:'obs',    title:'Rajasthan & Odisha Lead by Kg',
    ...
    { tag:'reason', title:'Northwest + Northeast Corridor Logic',
    ...
    { tag:'impact', title:'Persistent vs Emerging Hotspots',
    ...
    { tag:'obs',    title:'Geographic Concentration is Extreme',
    ...
    { tag:'reason', title:'Risk Index vs Volume Tell Different Stories',
  ...
  drug: [
    { tag:'obs',    title:'Ganja is Dominant by an Order of Magnitude',
    ...
    { tag:'reason', title:'Synthetic Drugs Growing in Share',
    ...
    { tag:'impact', title:'Correlation Matrix Reveals Trafficking Co-routes',
    ...
    { tag:'obs',    title:'Most State-Years Show Low Drug Diversity',
  ...
  enforcement: [
    { tag:'obs',    title:'Arrests Typically Exceed Cases (Rate > 1)',
    ...
    { tag:'reason', title:'High Variance in Enforcement Efficiency',
    ...
    { tag:'impact', title:'Cases and Arrests Rise in Parallel Nationally',
    ...
    { tag:'obs',    title:'Scatter Reveals Operation-Driven Outliers',
  ...
  border: [
    { tag:'obs',    title:'Border States Drive Over 50% of Seizure Volume',
    ...
    { tag:'reason', title:'Coastal States Underperform Relative to Risk',
    ...
    { tag:'impact', title:'Dual-Risk States (Border+Coastal) are Critical Nodes',
    ...
    { tag:'obs',    title:'Inland States Have Significant Seizures Too',
  ...
  advanced: [
    { tag:'obs',    title:'Heavy-Tailed Distribution — Few Events Dominate',
    ...
    { tag:'reason', title:'Pareto Effect in State Seizure Volumes',
    ...
    { tag:'impact', title:'Z-Score Anomalies Identify Events Worth Investigating',
    ...
    { tag:'obs',    title:'Growth Hotspots Are Geographically Diverse',
