...
const PL = {
  ...
  legend: { bgcolor: 'rgba(15,22,35,0.9)', bordercolor: '#1e2d45', borderwidth: 1 },
  hoverlabel: { bgcolor: '#0f1623', bordercolor: '#f97316', font: { color: '#e2eaf5' } },
...
document.querySelectorAll('.nav-btn').forEach(btn => {
  btn.addEventListener('click', () => {
    ...
    if (btn.dataset.page === 'statewise' && !window._stateInit) { initStateMap(); window._stateInit = true; }
    if (btn.dataset.page === 'eda'       && !window._edaInit)   { initEDA();      window._edaInit   = true; }
  });
});
...
function sumBy(data, col) { return data.reduce((s,d) => s + (d[col] || 0), 0); }
function groupYear(col)   { return YEARS.map(y  => sumBy(RAW_DATA.filter(d => d.Year     === y), col)); }
function groupState(col)  { return STATES.map(s => sumBy(RAW_DATA.filter(d => d.State_UT === s), col)); }
...
function populateHeroStats() {
  ...
  [
    ...
    { id:'hs-arrested', num:fmtNum(totalArrested), label:'Persons Arrested'   },
...
function renderHome() {
  ...
  Plotly.newPlot('home-trend', [{
    x: YEARS, y: kgByYear, type:'scatter', mode:'lines+markers',
    line: { color:'#f97316', width:3 },
    marker:{ size:9, color:'#f97316', line:{color:'#fff',width:2} },
    fill:'tozeroy', fillcolor:'rgba(249,115,22,0.1)',
    hovertemplate:'<b>Year %{x}</b><br>Seized: %{y:,.0f} Kg<extra></extra>'
  }], {
    ...PL,
    title:{ text:'National Drug Seizure Trend (2018–2024)', font:{size:15} },
    xaxis:{ ...PL.xaxis, title:'Year', dtick:1, tickformat:'d' },
    yaxis:{ ...PL.yaxis, title:'Total Weight Seized (Kg)' }
  }, PC);

  /* ② TOP 10 STATES */
  const stateKg = STATES
    .map(s => ({ s, v:sumBy(RAW_DATA.filter(d=>d.State_UT===s),'Total_Kg') }))
    .sort((a,b)=>b.v-a.v).slice(0,10);

  Plotly.newPlot('home-topstates', [{
    x: stateKg.map(d=>d.s), y:stateKg.map(d=>d.v), type:'bar',
    marker:{ color:stateKg.map((_,i)=>`hsl(${195+i*14},75%,52%)`), opacity:0.9 },
    hovertemplate:'<b>%{x}</b><br>%{y:,.0f} Kg seized<extra></extra>'
  }], {
    ...PL,
    title:{ text:'Top 10 States — Total Seizure Weight (All Years)', font:{size:15} },
  ...
  const nbKg = sumBy(RAW_DATA.filter(d=>!d.Border_State),'Total_Kg');
  Plotly.newPlot('home-border',[{
    values:[bKg,nbKg], labels:['Border States','Non-Border States'],
    type:'pie', hole:0.48,
    ...
    ...PL,
    title:{ text:'Border vs Non-Border — Share of Seizures (Kg)', font:{size:15} },
    margin:{t:55,b:20,l:20,r:20}
  }, PC);

  /* ④ TOP DRUGS HORIZONTAL BAR */
  const drugTotals = DRUG_KG_COLS
    .map(d=>({ label:d.label, val:sumBy(RAW_DATA, d.col) }))
    .filter(d=>d.val>0)
    .sort((a,b)=>b.val-a.val)
    .slice(0,10);

  Plotly.newPlot('home-drugs',[{
    y:drugTotals.map(d=>d.label), x:drugTotals.map(d=>d.val),
    type:'bar', orientation:'h',
    marker:{ color:drugTotals.map((_,i)=>`hsl(${80+i*8},70%,55%)`), opacity:0.88 },
    hovertemplate:'<b>%{y}</b><br>%{x:,.0f} Kg total<extra></extra>'
  }],{
    ...PL,
    title:{ text:'Top Drug Types by Total Seized Quantity (Kg, 2018–2024)', font:{size:15} },
...
function initStateMap() {
  const sorted = STATES
    .map(s => {
      ...
      return { s, risk:rows.reduce((sum,d)=>sum+d.Risk_Index,0).toFixed(4), kg:sumBy(rows,'Total_Kg') };
    })
  ...
  Plotly.newPlot('india-map',[{
    ...
    marker:{
      ...
      colorbar:{ title:{text:'Risk Index',side:'right',font:{color:'#5a7291'}}, tickfont:{color:'#5a7291'} }
    },
    hovertemplate:'<b>%{x}</b><br>Risk Index: %{y:.3f}<br>Total Kg: %{customdata:,.0f}<extra></extra>'
  }],{
    ...PL,
    title:{ text:'State Risk Index — Click a bar to load state detail below', font:{size:15} },
    xaxis:{ ...PL.xaxis, title:'State / UT', tickangle:-40, tickfont:{size:10} },
  ...
  document.getElementById('india-map').on('plotly_click', data => {
    ...
    applyStateFilters();
    document.querySelector('.state-controls').scrollIntoView({ behavior:'smooth', block:'center' });
  });
}
...
function applyStateFilters() {
  ...
  const gr = (row.Growth_Rate_Pct !== undefined ? row.Growth_Rate_Pct : row.Growth_Rate*100).toFixed(1);
  document.getElementById('risk').textContent      = row.Risk_Index.toFixed(4);
  document.getElementById('arrest').textContent    = row.Arrest_Rate.toFixed(2)+'x';
  document.getElementById('growth').textContent    = gr+'%';
  document.getElementById('diversity').textContent = row.Drug_Diversity+' types';
  ...
  document.getElementById('kpi-qty').textContent      = row.Total_Kg.toLocaleString(undefined,{maximumFractionDigits:1})+' Kg';
  ...
  // Drug table
  const found = ALL_DRUG_COLS
    .map(d=>({ label:d.label, unit:d.unit||'Kg', val:row[d.col]||0 }))
    ...
    ? `<table class="drug-table">
         ...
         <tbody>${found.map((r,i)=>`
           <tr><td>${i+1}</td><td>${r.label}</td><td>${r.unit}</td>
               <td>${r.val.toLocaleString(undefined,{maximumFractionDigits:2})}</td></tr>`).join('')}
         </tbody></table>`
    : '<p class="placeholder-text">No drug quantities recorded for this state/year.</p>';

  // Trend chart
  const rows = RAW_DATA.filter(d=>d.State_UT===state).sort((a,b)=>a.Year-b.Year);
  Plotly.newPlot('state-trend',[
    { x:rows.map(d=>d.Year), y:rows.map(d=>d.Total_Kg),
      name:'Total Kg', type:'scatter', mode:'lines+markers',
      line:{color:'#f97316',width:3}, marker:{size:9},
      hovertemplate:'<b>%{x}</b><br>Kg Seized: %{y:,.1f}<extra></extra>' },
    { x:rows.map(d=>d.Year), y:rows.map(d=>d.Total_Cases),
      name:'Cases Filed', type:'scatter', mode:'lines+markers',
      line:{color:'#38bdf8',width:2,dash:'dot'}, marker:{size:7}, yaxis:'y2',
      hovertemplate:'<b>%{x}</b><br>Cases: %{y:,}<extra></extra>' },
    { x:rows.map(d=>d.Year), y:rows.map(d=>d.Total_Arrested),
      name:'Arrested', type:'scatter', mode:'lines+markers',
      line:{color:'#a3e635',width:2,dash:'dash'}, marker:{size:7}, yaxis:'y2',
      hovertemplate:'<b>%{x}</b><br>Arrested: %{y:,}<extra></extra>' }
  ],{
    ...PL,
    title:{ text:`${state} — Year-on-Year Performance`, font:{size:15} },
    xaxis: { ...PL.xaxis, title:'Year', dtick:1, tickformat:'d' },
    yaxis: { ...PL.yaxis, title:'Seizure Weight (Kg)' },
    yaxis2:{ ...PL.yaxis, title:'Cases / Arrests (persons)', overlaying:'y', side:'right' },
